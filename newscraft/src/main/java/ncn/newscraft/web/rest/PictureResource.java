...
/**
 * REST controller for managing {@link ncn.newscraft.domain.Picture}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<Picture> result = pictureRepository
            ...
            .map(existingPicture -> {
                if (picture.getImageURL() != null) {
                    existingPicture.setImageURL(picture.getImageURL());
                }
                if (picture.getCaption() != null) {
                    existingPicture.setCaption(picture.getCaption());
                }
