    ...
    <div>
      ...
      <div className="table-responsive">
        {bookMarkList && bookMarkList.length > 0 ? (
            ...
            <tbody>
              {bookMarkList.map((bookMark, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{bookMark.createdBy ? <Link to={`/user-profile/${bookMark.createdBy.id}`}>{bookMark.createdBy.login}</Link> : ''}</td>
                  <td>{bookMark.linksTo ? <Link to={`/news-article/${bookMark.linksTo.id}`}>{bookMark.linksTo.title}</Link> : ''}</td>
          ...
          !loading && <div className="alert alert-warning">No Book Marks found</div>
        )}
      </div>
    </div>
  );
